...
const decode = (s) =>
  String(s).replace(/&(amp|lt|gt|quot|#39);/g, (m, e) => HTML_ENTITIES[e] ?? m);

/**
 * Reads every field out of a rendered admin form.
 *
 * An update writes all of a resource's declared columns, so posting a subset
 * blanks the rest - exactly what a browser avoids by submitting the whole form.
 * Tests that edit one field must do the same.
 */
function readForm(html) {
  const body = html.slice(html.indexOf('<form class="admin-form"'));
  const values = {};

  for (const [, attrs] of body.matchAll(/<input\b([^>]*)>/g)) {
    const name = (/\bname="([^"]*)"/.exec(attrs) || [])[1];
    if (!name) continue;
    const type = (/\btype="([^"]*)"/.exec(attrs) || [, 'text'])[1];
    const value = decode((/\bvalue="([^"]*)"/.exec(attrs) || [, ''])[1]);

    // A checkbox ships as a hidden "0" plus the box itself; the box only counts
    // when checked, which is how the unchecked state reaches the server as 0.
    if (type === 'checkbox') {
      if (/\bchecked\b/.test(attrs)) values[name] = value;
      else if (values[name] === undefined) values[name] = '0';
      continue;
    }
  ...
  for (const [, attrs, content] of body.matchAll(/<textarea\b([^>]*)>([\s\S]*?)<\/textarea>/g)) {
    const name = (/\bname="([^"]*)"/.exec(attrs) || [])[1];
    if (name) values[name] = decode(content);
  }

  for (const [, attrs, options] of body.matchAll(/<select\b([^>]*)>([\s\S]*?)<\/select>/g)) {
    const name = (/\bname="([^"]*)"/.exec(attrs) || [])[1];
    if (!name) continue;
    const selected = /<option value="([^"]*)"[^>]*\bselected\b/.exec(options);
    values[name] = selected ? decode(selected[1]) : '';
  }

  return values;
}

/** Submits an admin form with `overrides` applied, preserving every other field. */
async function submitForm(editUrl, postUrl, overrides) {
  const html = await (await request(editUrl)).text();
  const values = { ...readForm(html), ...overrides };
  return request(postUrl, { method: 'POST', headers: POST_FORM, body: form(values) });
}
...
async function main() {
  ...
  {
    // A rendered page must never be cached, or an edit made in the admin looks
    // like it did not take effect.
    const res = await request('/portfolio.html');
    assert.match(
      res.headers.get('cache-control') || '',
      /no-store/,
      'rendered pages are cacheable, so edits can appear not to apply'
    );
    ok('rendered pages are served no-store');
  }
  ...
  console.log('\n== project detail pages');
  {
    ...
    // The cover images were exported at seven different aspect ratios, so the
    // cards only come out the same size if the override stylesheet is loaded
    // and pins one ratio. Guard both halves of that.
    assert(
      grid.includes('href="css/site-overrides.css"'),
      'portfolio.html no longer loads the card-sizing stylesheet'
    );
    const overrides = await request('/css/site-overrides.css');
    assert.strictEqual(overrides.status, 200, 'card-sizing stylesheet is not served');
    const css = await overrides.text();
    assert(/--card-thumb-ratio/.test(css), 'card ratio variable missing');
    assert(
      /\.alt-portfolio-item\.card-portfolio \.alt-portfolio-thumb > img \{[^}]*aspect-ratio/.test(css),
      'portfolio card images are not pinned to one aspect ratio'
    );
    assert(/object-fit: cover/.test(css), 'card images would distort instead of crop');
    ok('every portfolio card is pinned to one aspect ratio');
    ...
    // Every project page must have the same structure as Krooqi's. The
    // testimonial is the deliberate exception: it is opted into per project,
    // because attaching an existing quote would misattribute a real person.
    const SHARED_BLOCKS = [
      ['hero image', 'col-12 pt-30'],
      ['second image', 'col-lg-5 pr-100'],
      ['introduction box', '>Introduction<'],
      ['solution', '>The Solution<'],
      ['key features', '<li class="neutral-950">'],
      ['outcome', '>Outcome<'],
      ['gallery', 'about-me-slider-active'],
      ['closing image', 'col-12 pb-50'],
      ['related projects', 'Related projects']
    ];

    for (const slug of slugs) {
      const html = await (await request(`/portfolio-details.html?slug=${slug}`)).text();
      for (const [name, marker] of SHARED_BLOCKS) {
        assert(html.includes(marker), `${slug} is missing the ${name} block`);
      }
    }
    ok(`all ${slugs.length} project pages share the same ${SHARED_BLOCKS.length} structural blocks`);
  ...
  {
    // Sections still collapse when a project genuinely has nothing in them,
    // rather than rendering a heading over empty space.
    {
      const list = await (await request('/admin/portfolio-projects?q=mamame')).text();
      const id = Number(/data-id="(\d+)"/.exec(list)[1]);
      const editUrl = `/admin/portfolio-projects/${id}/edit`;
      const postUrl = `/admin/portfolio-projects/${id}`;

      // Capture the whole form so the restore puts every field back, not just
      // the three this check changes.
      const original = readForm(await (await request(editUrl)).text());
      onCleanup(async () => {
        // Drop the captured token so the restore uses a freshly issued one.
        const { _csrf: _stale, ...fields } = original;
        await submitForm(editUrl, postUrl, fields);
      });

      await submitForm(editUrl, postUrl, {
        solution_text: '',
        features: '',
        outcome_text: ''
      });

      const emptied = await (await request('/portfolio-details.html?slug=mamame')).text();
      assert(!emptied.includes('>The Solution<'), 'emptied solution block still rendered');
      assert(!emptied.includes('>Outcome<'), 'emptied outcome block still rendered');
      ok('a section still collapses when its content is cleared');
    }
  }
