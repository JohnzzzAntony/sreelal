...
const config = {
  ...
  // `path.resolve` rather than `path.join`: these accept an absolute path as
  // well as one relative to the project, and join() would append an absolute
  // value to the project root instead of honouring it. Containers and mounted
  // volumes pass absolute paths.
  paths: {
    root: ROOT,
    site: path.join(ROOT, 'IAMSREE'),
    views: path.join(__dirname, 'views'),
    data: path.resolve(ROOT, process.env.DATA_DIR || 'data'),
    uploads: path.resolve(ROOT, process.env.UPLOAD_DIR || path.join('IAMSREE', 'images', 'uploads'))
  },
  ...
  db: {
    file: path.resolve(ROOT, process.env.DATABASE_PATH || path.join('data', 'site.db'))
  },
